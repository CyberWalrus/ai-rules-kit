...
```bash
# 🔍 КАЧЕСТВЕННАЯ ПРОВЕРКА (ALL-IN-ONE) - рекомендуется!
npm run quality:check
# КРИТИЧНО: Запускает ВСЕ проверки параллельно перед коммитом!
# Включает: lint, lint:unused, typecheck, test:unit, test:e2e
# Единый отчет с детальными ошибками за ~7 секунд
# Использование: yarn run quality:check или npm run quality:check
...
- **quality:check** → Все проверки за 7 секунд = быстрая обратная связь = уверенность в коде
...
Выполнение `npm run quality:check` перед каждым коммитом = статус Senior Developer = уважение команды!

**⚡ ПРЕИМУЩЕСТВА quality:check:**

- Параллельное выполнение всех проверок (экономия времени в 2-3 раза)
- Единый отчет с цветным выводом (✅/❌)
